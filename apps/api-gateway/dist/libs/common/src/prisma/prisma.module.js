...
exports.prismaToken = prismaToken;
...
function prismaToken(serviceName) {
...
let PrismaModule = PrismaModule_1 = class PrismaModule {
    ...
    static forServices(...serviceNames) {
        const providers = serviceNames.map((name) => ({
            provide: prismaToken(name),
            useFactory: () => (0, prisma_config_1.createPrismaClient)(name),
        }));
        return {
            module: PrismaModule_1,
            providers,
...
function createServiceModule(serviceName, serviceProviders) {
    ...
    return {
        ...
        exports: [...serviceProviders.map((p) => p.provide || p)],
    };
}
